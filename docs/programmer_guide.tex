...
The full 63-configuration sweep was run on the following hardware:
...
Estimated runtime per configuration is 1.5--2 hours on equivalent hardware.
A GPU with at least 16\,GB VRAM is recommended for retraining; the default
batch size of 4 allows training to run on GPUs with as little as 8\,GB VRAM
at the cost of slower convergence. A free-tier Colab T4 (16\,GB) is sufficient
for training individual configurations but is not practical for the full sweep
due to session time limits.
...
\begin{enumerate}
    \item A Google account with access to the project Google Drive folder.
          ...
          which packages the best-performing Control and TC checkpoints
          selected for the demo. The complete weight archive covering all
          70 experiment configurations (63 TC + 7 Control seeds) is
          available on request from \texttt{ishay.yulzary@gmail.com}.
